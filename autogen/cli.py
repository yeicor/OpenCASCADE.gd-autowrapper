...
def _run_cli(argv: list[str]) -> int:
    """Run another autogen subcommand as a subprocess (like generate.sh does).

    Running the pipeline stages as separate processes keeps each libclang scan
    worker in its own freshly-forked pool -- the exact layout generate.sh uses
    -- instead of pooling inside a process that has already touched libclang.
    """
...
def cmd_regenerate(args: argparse.Namespace) -> int:
    """Full generate.sh pipeline in one invocation (used by CMake and CI).

    Fresh scan of every OCCT module, then repeated generate + audit passes
    until the symbol probe compiles clean (or the audit is unavailable for a
    cross target), writing a stamp file on success so the CMake step can skip
    already-fresh wrappers on subsequent configures.
    """
    from .occt import find_occt_install

    # Mirror generate.sh's graceful degradation: no OCCT headers -> nothing to
    # do (CI test/export jobs only consume prebuilt artifacts).
    try:
        install = find_occt_install(PROJECT_ROOT)
    except FileNotFoundError as e:
        print(f"regenerate     : {e}", file=sys.stderr)
        print("regenerate     : no OCCT install found; skipping generation", file=sys.stderr)
        return 0
    try:
    ...
    for i in range(1, args.max_passes + 1):
        print(f"generating wrappers + symbol probe (pass {i})...", flush=True)
        rc = _run_cli(["generate-all", "--irs", *irs,
                       "--out", str(wrapper_out),
                       "--probe-out", str(args.probe_out),
                       "--missing", str(missing),
                       "--illformed", str(illformed),
                       "--synth-cache", str(args.synth_cache)])
        if rc != 0:
            return rc
        rc = _run_cli(["audit", "--irs", *irs,
                       "--probe", str(args.probe_out),
                       "--work", str(args.work),
                       "--out", str(args.missing_cur),
                       "--illformed-out", str(args.illformed_cur),
                       "--compile-db", str(args.compile_db),
        ...
        n_missing = cur_missing.stat().st_size if cur_missing.exists() else 0
        n_ill = cur_ill.stat().st_size if cur_ill.exists() else 0
        if n_missing == 0 and n_ill == 0:
            break
        for acc, cur in ((missing, cur_missing), (illformed, cur_ill)):
            if cur.exists() and cur.stat().st_size:
                _merge_skip_set(acc, cur)
        print(f"regenerate     : pass {i} findings merged; regenerating...",
              flush=True)

    stamp = Path(args.stamp)
    stamp.parent.mkdir(parents=True, exist_ok=True)
    stamp.write_text(f"occt={install.version}\n"
                     "generated by `python -m autogen regenerate`\n")
    print(f"regenerate     : stamped {stamp}")
    return 0


def _merge_skip_set(acc: Path, cur: Path) -> None:
    """Append the current pass's findings into the accumulated skip set (like
    generate.sh's `cat ... | sort -u`)."""
    cur_lines = cur.read_text().splitlines()
    if not cur_lines:
        return
    acc_lines = set(acc.read_text().splitlines()) if acc.exists() else set()
    merged = sorted(acc_lines | set(cur_lines))
    acc.write_text("\n".join(merged) + "\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_reg = sub.add_parser(
        "regenerate", help="scan-all + iterate generate/audit passes (generate.sh)")
    p_reg.add_argument("--jobs", type=int, default=_default_jobs())
    p_reg.add_argument("--out", type=Path,
                       default=PROJECT_ROOT / "src" / "autowrapper",
                       help="wrapper output dir (default: <repo>/src/autowrapper)")
    p_reg.add_argument("--ir-out", type=Path, default=SUBMODULE_DIR / "out" / "ir")
    p_reg.add_argument("--probe-out", type=Path,
                       default=SUBMODULE_DIR / "out" / "audit" / "probe.cpp")
    p_reg.add_argument("--work", type=Path, default=SUBMODULE_DIR / "out" / "audit")
    p_reg.add_argument("--missing-acc", type=Path,
                       default=SUBMODULE_DIR / "out" / "audit" / "skips-missing.txt")
    p_reg.add_argument("--illformed-acc", type=Path,
                       default=SUBMODULE_DIR / "out" / "audit" / "skips-illformed.txt")
    p_reg.add_argument("--missing-cur", type=Path,
                       default=SUBMODULE_DIR / "out" / "audit" / "missing.txt")
    p_reg.add_argument("--illformed-cur", type=Path,
                       default=SUBMODULE_DIR / "out" / "audit" / "illformed.txt")
    p_reg.add_argument("--synth-cache", type=Path,
                       default=SUBMODULE_DIR / "out" / "synth" / "specs.json")
    p_reg.add_argument("--max-passes", type=int, default=6)
    p_reg.add_argument("--stamp", type=Path,
                       default=PROJECT_ROOT / "src" / "autowrapper" / ".autowrapper-stamp")
    p_reg.add_argument("--compile-db", type=Path, default=DEFAULT_COMPILE_DB)
    p_reg.add_argument("--compiler", type=str, default="g++")
    p_reg.add_argument("--nm", type=str, default="nm")
    p_reg.set_defaults(func=cmd_regenerate)
