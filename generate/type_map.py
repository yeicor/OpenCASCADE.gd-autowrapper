...
# Bnd_Box::Limits / Bnd_Box2d::Limits are nested structs of doubles returned by
# value from Bnd_Box::Get() / Bnd_Box2d::Get(). They are packed into a
# PackedFloat64Array in the wrapper.
BND_LIMITS_MEMBERS = {
    "Bnd_Box::Limits": ("Xmin", "Xmax", "Ymin", "Ymax", "Zmin", "Zmax"),
...
def bnd_limits_members_for_type(otype: OCCTType):
    ...
    or None. Tries base_name, canonical spelling, and raw spelling."""
    for candidate in (otype.base_name, otype.canonical_spelling or "",
                      otype.spelling.replace("&", "").strip()):
        if candidate in BND_LIMITS_MEMBERS:
            return BND_LIMITS_MEMBERS[candidate]
    return None
...
class TypeMap:
    ...
    def class_decl(self, occt_name: str) -> ClassDecl | None:
        """Get the ClassDecl for a wrapped OCCT type, or None."""
        return self._classes.get(occt_name)
    ...
    def cpp_type_for_return(self, otype: OCCTType | None) -> str:
        ...
        # Bnd_*::Limits nested double-struct returns → PackedFloat64Array
        if bnd_limits_members_for_type(otype):
            return "PackedFloat64Array"
